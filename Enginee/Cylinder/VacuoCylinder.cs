    ...
    public class VacuoCylinder : Cylinder,ICylinderStatusJugger
    {
        ...
        public IList<Alarm> Alarms
        {
            get
            {
                ...
                list.Add(new Alarm(() => NoSensorAlarm())
                {
                    AlarmKey = string.Format("{0}无感应信号报警！", Name),
                    AlarmLevel = AlarmLevels.Error,
                    Name = string.Format("{0}无感应信号报警！", Name)
                });
                list.Add(new Alarm(() => OffSensorAlarm())
                {
                    AlarmKey = string.Format("{0}感应信号常量报警！", Name),
                    AlarmLevel = AlarmLevels.Error,
                    Name = string.Format("{0}感应信号常量报警！", Name)
                });
