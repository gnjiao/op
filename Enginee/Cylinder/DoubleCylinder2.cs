...
namespace System.Enginee
{
    ...
    public class DoubleCylinder2 : Cylinder, ICylinderStatusJugger
    {
        ...
        public DoubleCylinder2(IoPoint InOrigin, IoPoint InMove, IoPoint OutOrigin, IoPoint OutMove)
        {
            _InOrigin = InOrigin;
            _InMove = InMove;
        ...
        public IList<Alarm> Alarms
        {
            get
            {
                ...
                list.Add(new Alarm(() => NoOriginAlarm())
                {
                    AlarmKey = string.Format("{0}无原点报警！", Name),
                    AlarmLevel = AlarmLevels.Error,
                    Name = string.Format("{0}无原点报警！", Name)
                });
                list.Add(new Alarm(() => NoMoveAlarm())
                {
                    AlarmKey = string.Format("{0}无动点报警！", Name),
                    AlarmLevel = AlarmLevels.Error,
                    Name = string.Format("{0}无动点报警！", Name)
                });
                list.Add(new Alarm(() => OnOriginAlarm())
                {
                    AlarmKey = string.Format("{0}ON时，原点信号报警！", Name),
                    AlarmLevel = AlarmLevels.Error,
                    Name = string.Format("{0}ON时，原点信号报警！", Name)
                });
                list.Add(new Alarm(() => OffMoveAlarm())
                {
                    AlarmKey = string.Format("{0}OFF时，动点信号报警！", Name),
                    AlarmLevel = AlarmLevels.Error,
                    Name = string.Format("{0}OFF时，动点信号报警！", Name)
                });
                list.Add(new Alarm(() => AutoOnPrompt())
                {
                    AlarmKey = string.Format("{0}自动时，手动为ON提示！", Name),
                    AlarmLevel = AlarmLevels.None,
                    Name = string.Format("{0}自动时，手动为ON提示！", Name)
                });
                return list;
            }
        }
        ...
        public void StatusJugger()
        {
            ...
            if (_OutOrigin.Value && _InOrigin.Value) { _OutOrigin.Value = false; }
            if (_OutMove.Value && _InMove.Value) { _OutMove.Value = false; }
        }
        #endregion
    }
}
